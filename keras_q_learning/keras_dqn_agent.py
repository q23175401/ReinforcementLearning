...
class DqnAgent(QBaseAgent):
    ...
    def train_one_batch(self, batch_size, DISCOUNT_FACTOR):
        ...
        max_future_q_values = np.max(next_q_target, axis=1) # find max futrue values of each next state
        
        for ti, traj_data in enumerate(one_batch):
            _, action, reward, _, done = traj_data

            target_q = reward if done else reward + DISCOUNT_FACTOR * max_future_q_values[ti]
